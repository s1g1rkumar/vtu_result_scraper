document.addEventListener("DOMContentLoaded", () => {
  ...
  fetchButton.addEventListener("click", async () => {
    ...
    setLoading(true);
    updateStatus(
      `⏳ Starting scrape for **${usnList.length} USNs**... This may take a moment due to CAPTCHA solving.`,
      "initial",
      true 
    );
    ...
    try {
      ...
      if (data.total_successful > 0) {
        let statusMsg = `✅ **COMPLETED!** Processed ${data.total_requested} USNs. Successful: **${data.total_successful}**`;
        ...
        updateStatus(
          `❌ **FAILURE!** Could not retrieve results for any of the ${data.total_requested} USNs. Reasons may include invalid USN range, incorrect URLs, or persistent CAPTCHA errors.`,
          "failure"
        );
      }
